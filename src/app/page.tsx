...
function RideBooking({ currentPosition, onDestinationSelect, destinationName, fare, onConfirmRide }) {
  return (
    <CardContent className="p-4 space-y-4">
      <DestinationSearch onSelect={onDestinationSelect} currentPosition={currentPosition} />
      {destinationName && (
        <>
          <Separator />
          ...
          <Button size="lg" className="w-full text-lg" onClick={onConfirmRide}>
            Confirm Ride
          </Button>
        </>
      )}
...
function DestinationSearch({ onSelect, currentPosition }: { onSelect: (place: any, name?: string) => void, currentPosition: LatLngTuple | null }) {
  ...
  const handleAiSuggestionSelect = async (suggestion: SuggestDestinationOutput) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        handleSelect(data[0], suggestion.suggestedDestination);
      } else {
         toast({
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="max-h-60 overflow-y-auto">
        ...
        {suggestions.map(p => (
            <button key={p.place_id} className="w-full text-left p-3 flex items-center gap-4 hover:bg-secondary rounded-lg" onClick={() => handleSelect(p, p.display_name)}>
